  ...
  actions: {
    ...
    async refreshSchema() {
      ...
      try {
        ...
        if (data.tables) this.tables = data.tables.map((t: string) => ({ name: t, rowCount: 100, type: 'table' }))
        if (data.views) this.views = data.views.map((v: string) => ({ name: v, type: 'view' }))
        if (data.functions) this.functions = data.functions.map((f: string) => ({ name: f, type: 'function' }))
      } catch (e) {
